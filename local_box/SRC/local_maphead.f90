    ...
    subroutine loc_maphead
    use common_icos_loc_box
    implicit none

    mrc%nx=sub_FFTsize  !        /* # of columns ( fastest changing in the map    */
    mrc%ny=sub_FFTsize          !  /* # of rows                                     */
    mrc%nz=sub_FFTsize           ! /* # of sections (slowest changing in the map    */
    ...
    mrc%nxstart=-sub_FFTsize/2      !/* number of first column in map (default = 0)   */
    mrc%nystart=-sub_FFTsize/2       !/* number of first row in map (default = 0)      */
    mrc%nzstart=-sub_FFTsize/2       !/* number of first ssection in map (default = 0) */

    mrc%mx=sub_FFTsize-1           !/* number of intervals along X                   */
    mrc%my=sub_FFTsize-1            !/* number of intervals along Y                   */
    mrc%mz=sub_FFTsize-1            !/* number of intervals along Z                   */
