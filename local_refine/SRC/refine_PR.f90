    ...
    do pID=first,last
        ...
        if((proc2d%applyCTF.eq.'y').or.(proc2d%applyCTF.eq.'Y')) then
            !if(particle(pID)%imgID.ne.imgID0) then
               call getCTF2d(FFTsize,particle(pID)%df1,particle(pID)%df2,particle(pID)%astigang,  &
...
!                df1=particle(pID)%df1+particle(pID)%local_z*0.0001
!                df2=particle(pID)%df2+particle(pID)%local_z*0.0001
!                call getCTF2d(FFTsize,df1,df2,particle(pID)%astigang,  &
!                            apix,CTFpar%vol,CTFpar%Cs,CTFpar%ampwg,ctf2d)
!            else
!                if((proc2d%particle_df.eq.'n').or.(proc2d%particle_df.eq.'N')) then
!                    if(particle(pID)%imgID.ne.imgID0) then
!                        call getCTF2d(FFTsize,particle(pID)%df1,particle(pID)%df2,particle(pID)%astigang,  &
!                            apix,CTFpar%vol,CTFpar%Cs,CTFpar%ampwg,ctf2d)
!                        imgID0=particle(pID)%imgID
!                    end if
!                else
!                    if( (particle(pID)%imgID.ne.imgID0) .or. (particle(pID)%imgparticleID .ne. imgparticleID0) ) then
!                        call getCTF2d(FFTsize,particle(pID)%df1,particle(pID)%df2,particle(pID)%astigang,  &
!                            apix,CTFpar%vol,CTFpar%Cs,CTFpar%ampwg,ctf2d)
!                        imgID0=particle(pID)%imgID
!                        imgparticleID0=particle(pID)%imgparticleID
...
!        end if
        ...
        particle(pID)%local_z     =particle(pID)%local_z0
        ...
        write(41,901) particle(pID)%stckfile,particle(pID)%particleID,particle(pID)%theta,particle(pID)%phi,particle(pID)%omega, &
                ...
                particle(pID)%local_dx,particle(pID)%local_dy
