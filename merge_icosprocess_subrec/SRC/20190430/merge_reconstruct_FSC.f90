...
subroutine merge_reconstruct_FSC
use common_merge_icosprocess_subrec
    implicit none
    integer i,hx,ky,lz,n1,n2,n3,i3,i3a,i3b,ir,tnfsize,mapsize,tmp
    real FSC1(0:maxR),FSC2(0:maxR),FSC(0:maxR),tmp2
    real,allocatable::a0(:),b0(:),wg0(:),a1(:),b1(:),a2(:),b2(:),wg1(:),wg2(:),fa(:),fb(:)

    open(13,file='FSC.txt')
    print*,'calculating FSC...'
    tnfsize=(maxR+1)*(2*maxR+1)**2
    mapsize=FFTsize**3
    allocate(a0(tnfsize),b0(tnfsize),wg0(tnfsize),a1(tnfsize),b1(tnfsize),a2(tnfsize),b2(tnfsize),wg1(tnfsize),wg2(tnfsize))

    a1=.0;b1=.0;wg1=.0
    a2=.0;b2=.0;wg2=.0
    do i=1,nfile-1
        open(11,file=trim(par(i))//'oddtnf', form='unformatted',access='stream',status='old')
        open(12,file=trim(par(i))//'evetnf',form='unformatted',access='stream',status='old')
...
!    deallocate(wg1)
    ...
    call maphead
    open(21,file=trim(par(i)),form='unformatted',access='stream')
    write(21) mrc

    if(nicosavg.eq.1) call img3d_icosavg(fa,FFTsize,imgmask)
