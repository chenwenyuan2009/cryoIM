...
use FFTW3SPACE
    ...
    integer tmp,dr
    real,allocatable::fa(:),fb(:),wg(:),fa1(:),fb1(:),wg1(:),fa0(:),fb0(:),wg0(:),fina(:)
    ...
    allocate(fa1(FFTsize**3),fina((FFTsize-8)**3))
    fa1=.0
    do i=1,tmaxR**3
    ...
    do i=1,(FFTsize-8)**3
        lz=i/(FFTsize-8)**2+4
        ky=mod(i-1,(FFTsize-8)**2)/(FFTsize-8)+4
        hx=mod(mod(i-1,(FFTsize-8)**2),FFTsize-8)+4
        n=lz*FFTsize**2+ky*FFTsize+hx+1
        fina(i)=fa1(n)
    end do
    call maphead
