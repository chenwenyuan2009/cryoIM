...
module common_icosproc
    ...
    integer FFTsize,geneFFTsize,first,last,totalimg,totalpars,minR,maxR,mode,Ntemplate,clipsize !
    integer totaltemplate,Nlin,Ncycle,useparticle,loc_box  !globe search and refinement
    integer inter_box,inter_w,Nsym,Padsize !reconstruction
    real apix,minRes,maxRes,imgmask,imask,particle_radius  !
    real deltaAngle,deltaTrans,searchstep !globe search and refinement
    real PR_threshold,PR_threshold_h !reconstruction
    real capsid_innerR,capsid_outerR,core_radius,centbound
    parameter(totalpars=49)
    character(len=128)::model3d,ortfile,templateort,newortfile,result3d,templatestck,core3d
    character(len=128)::capsidmodel,coremodel,geneimgstck,imgstck,newimgstck
    character(len=64) ::coeff(totalpars),par(50)
    character(len=54) ::stckfile,tnffile
    character(len=4)  ::sym

    type process2d
        character applyCTF,imgmask,norm,FRC,geneFFTsize,centshift,imgstck,newimgstck,ortfile,clipsize,pr_cc
    end type
    type(process2d)::proc2d

    type process3d
        character realspaceavg,model3d,templatestck,templateort,recISAF,imask,subrec,FSC
        character outputprojstck,outputrawimgstck,core3d
    end type
    ...
    type particles
        ...
        real theta, phi, omega, x, y, PR,deltdf1,deltdf2,deltdfa
