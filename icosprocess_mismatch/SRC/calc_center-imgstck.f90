...
use common_icosproc
    ...
    do i=first,last
        ...
        if(cc.le.-1.0 ) cc=-1.0
        if(isnan(cc))  cc=0.0
        if(abs(centx).ge.21. .or. isnan(centx)) centx=0.0
        if(abs(centy).ge.21. .or. isnan(centy)) centy=0.0
    ...
    if((proc2d%centshift.eq.'y').or.(proc2d%centshift.eq.'Y')) then
            call img2d_centshift_FT(a3,FFTsize,particle(i)%x,particle(i)%y)
            particle(i)%x=.0
            particle(i)%y=.0
        end if
