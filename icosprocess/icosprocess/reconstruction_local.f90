subroutine reconstruct_local
use common_icosproc
use FFTW3SPACE
    implicit none
    integer i,i2,j,hx,ky,lz,imgID0,no0,no1,n1,n2,imgparticle0
    integer usedparticle,signz,mapsize,ix,iy,ip1,ip2
    integer isym,lx(3),cubID(8),x10,y10
...
!************************************************************************************
                            ...
                            cubID(1)=lx(3)*loc_box**2+lx(2)*loc_box+lx(1)+1
                            cubID(2)=cubID(1)+1
                            cubID(3:4)=cubID(1:2)+loc_box
                            cubID(5:8)=cubID(1:4)+loc_box**2
                            do j=1,8
    ...
    do ky=1,maxR
        do hx=-maxR,maxR
            n1=( lz+loc_box/2)*loc_box**2+( ky+loc_box/2)*loc_box+( hx+loc_box/2)+1
            n2=(-lz+loc_box/2)*loc_box**2+(-ky+loc_box/2)*loc_box+(-hx+loc_box/2)+1
    ...
    call FFTcmplx3d_B(FFT3d,FFT3d,loc_box,loc_box,loc_box)
    call shift3dComplex(FFT3d,loc_box,loc_box,loc_box)
    fa=real(FFT3d)
    deallocate(FFT3d)
    ...
    if((proc3d%realspaceavg.eq.'Y').or.(proc3d%realspaceavg.eq.'y')) then
        call img3d_icosavg(fa,loc_box,imgmask)
    end if
    write(21) fa
...
!    end do
    close(21)
    deallocate(fa,a,b,shift2d,img2do)
    return
end