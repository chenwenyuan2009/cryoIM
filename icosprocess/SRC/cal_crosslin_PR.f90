...
subroutine cal_crosslin_PR(PR)
    ...
    real,allocatable::Fcut1(:),alpha1(:),alpha2(:),a1(:),b1(:),a2(:),b2(:),FT_commlin1(:,:,:),FT_commlin2(:,:,:)
    allocate(Fcut1(minR:maxR),alpha1(Nlin),alpha2(Nlin))
    allocate(a1(FFTsize**2),b1(FFTsize**2),a2(FFTsize**2),b2(FFTsize**2))
    allocate(FT_commlin1(2,minR:maxR,Nlin),FT_commlin2(2,minR:maxR,Nlin))
    ...
    deallocate(Fcut1,alpha1,alpha2,a1,b1,a2,b2,FT_commlin1,FT_commlin2)
    return
end
