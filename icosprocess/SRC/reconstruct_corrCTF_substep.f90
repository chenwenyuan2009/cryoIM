



subroutine reconstruct_corr_CTF_substep
use common_icosproc
    implicit none
    integer i,i2,j,hx,ky,imgID0,no0,no1,imgparticle0,tmp1,tmp2
    integer usedparticle,signz,mapsize,tmaxR
    integer isym,lx(3),cubID(8),defocuscycle
    real theta,phi,omega,dr,rot(3,3),x0(3),x(3),x00(3)
    real matsym(3,3,60),ax(3),disx(8)
    real df1,df2,tmpx1,tmpx2,tmpx3
    real,allocatable::a(:),b(:),shift2d(:),ctf3d(:,:)
    real,allocatable::fa(:),fb(:),wg(:),img2dstck(:)

    open(21,file=trim(result3d), form='unformatted',access='stream',status='replace')
 
    tmaxR=2*maxR+1
    mapsize=(tmaxR)**2*(maxR+1)
    allocate(a(FFTsize**2),b(FFTsize**2),shift2d(FFTsize**2))
    allocate(fa(mapsize),fb(mapsize),wg(mapsize))
    allocate(ctf3d(FFTsize**2,-corrCTF_defocuscycle:corrCTF_defocuscycle))
    !boundX=FFTsize/4.0
    fa=.0
    fb=.0
    wg=.0
    imgID0=-100
    imgparticle0=-100
    usedparticle=0
    matsym=.0
    matsym(1,1,1)=1;matsym(2,2,1)=1;matsym(3,3,1)=1
    if(sym.eq.'icos') call icos2f_sym_mat(matsym)

    do i2=1,FFTsize**2
        ky=(i2-1)/FFTsize
        hx=mod(i2-1,FFTsize)
        shift2d(i2)=(-1)**(hx+ky)
    end do

    no0=-1
    stckfile='abcdef'
    allocate(img2dstck(FFTsize**2))
    if(proc2d%imgstck.eq.'y') then
        open(11,file=trim(imgstck),form='unformatted',access='stream',status='old')
        read(11) mrc
        do i=1,first-1
            call fseek(11,4*FFTsize**2,1)
        end do
    end if

    do i=first,last
!        print*,'i=',i
!**********************************************************************
        no1=int(float((i-first)*100)/float(last-first))
        if((no1.gt.no0).and.(mod(no1,10).eq.0)) then
            no0=no1
            write(*,101) no0,'%'
        end if
101     FORMAT(i7,a1,$)
!**********************************************************************
        if(proc2d%imgstck.eq.'y') then
            read(11) a
        else
            if(trim(stckfile).ne.trim(particle(i)%stckfile)) then
                deallocate(img2dstck)
                stckfile=trim(particle(i)%stckfile)
                open(11,file=trim(stckfile),form='unformatted',access='stream',status='old')
                read(11) mrc
                allocate(img2dstck(mrc%nx*mrc%ny*mrc%nz))
                read(11) img2dstck
                close(11)
            end if
            a(:)=img2dstck((particle(i)%imgparticleID-1)*FFTsize**2+1:particle(i)%imgparticleID*FFTsize**2)
        end if
        if((particle(i)%PR.le.PR_threshold).and.(abs(particle(i)%x).le.boundX ).and.(abs(particle(i)%y).le.boundX)) then
            usedparticle=usedparticle+1
            call img2d_norm(a,FFTsize)
            if((proc2d%applyCTF.eq.'y').or.(proc2d%applyCTF.eq.'Y')) then
                if(particle(i)%imgID.ne.imgID0) then
                    ctf3d=.0
                    do defocuscycle=-corrCTF_defocuscycle,corrCTF_defocuscycle
                        df1=particle(i)%df1+corrCTF_defocusstep*float(defocuscycle)
                        df2=particle(i)%df2+corrCTF_defocusstep*float(defocuscycle)
                        call getCTF2d(FFTsize,df1,df2,particle(i)%astigang,apix,CTFpar%vol,CTFpar%Cs,CTFpar%ampwg,ctf2d)
                        ctf3d(:,defocuscycle)=ctf2d(:)
                    end do
                    imgID0=particle(i)%imgID
                end if
            end if

            if((proc2d%imgmask.eq.'y').or.(proc2d%imgmask.eq.'Y')) call img2d_mask(a,FFTsize,imgmask,particle%x,particle%y)
            a=a*shift2d
            b=.0
            call img2d_FFT(a,b,FFTsize,FFTsize,1)
            call img2d_FT_centshift(a,b,FFTsize,particle(i)%x,particle(i)%y)

            theta=particle(i)%theta
            phi  =particle(i)%phi
            omega=particle(i)%omega
            call euler_mat(theta,phi,omega,rot)

            do ky=-maxR,maxR
                do hx=0,maxR
                    dr=sqrt(float(hx**2+ky**2))
                    if(dr.le.maxR) then
                        i2=(ky+FFTsize/2)*FFTsize+(hx+FFTsize/2)+1
                        x(1)=hx
                        x(2)=ky
                        x(3)=.0
                        x0=matmul(rot,x)
!************************************************************************************
                        x00=x0
                        do isym=1,Nsym
                            x=x00
                            x0=matmul(matsym(:,:,isym),x)
                            if(x0(3).lt..0) then
                                x0=-x0
                                signz=-1
                            else
                                signz=1
                            end if
                            x0(1:2)=x0(1:2)+maxR
!************************************************************************************
                            lx=int(x0)
                            ax=x0-lx
                            disx(1)=(1.0-ax(1))*(1.0-ax(2))*(1.0-ax(3))
                            disx(2)=(    ax(1))*(1.0-ax(2))*(1.0-ax(3))
                            disx(3)=(1.0-ax(1))*(    ax(2))*(1.0-ax(3))
                            disx(4)=(    ax(1))*(    ax(2))*(1.0-ax(3))
                            disx(5)=(1.0-ax(1))*(1.0-ax(2))*(    ax(3))
                            disx(6)=(    ax(1))*(1.0-ax(2))*(    ax(3))
                            disx(7)=(1.0-ax(1))*(    ax(2))*(    ax(3))
                            disx(8)=(    ax(1))*(    ax(2))*(    ax(3))
                            cubID(1)=lx(3)*tmaxR**2+lx(2)*tmaxR+lx(1)+1
                            cubID(2)=cubID(1)+1
                            cubID(3:4)=cubID(1:2)+tmaxR
                            cubID(5:8)=cubID(1:4)+tmaxR**2
                            do j=1,8
                                tmpx1=a(i2)*disx(j)
                                tmpx2=b(i2)*disx(j)
                                do defocuscycle=-corrCTF_defocuscycle,corrCTF_defocuscycle
                                    tmpx3=ctf3d(i2,defocuscycle)**2
                                    fa(cubID(j))=fa(cubID(j))+tmpx1*ctf3d(i2,defocuscycle)
                                    fb(cubID(j))=fb(cubID(j))+tmpx2*ctf3d(i2,defocuscycle)*signz
                                    wg(cubID(j))=wg(cubID(j))+      disx(j)*ctf2d(i2)**2
                                end do
                            end do
                        end do
                    end if
                end do
            end do
        end if
    end do
    tmp1=0
    tmp2=0
    if((proc3d%FSC.eq.'y').or.(proc3d%FSC.eq.'Y')) tmp1=1
    if((proc3d%realspaceavg.eq.'y').or.(proc3d%realspaceavg.eq.'Y')) tmp2=1
    write(21) FFTsize,maxR,apix,imgmask,tmp1,tmp2
    write(21) fa
    write(21) fb
    write(21) wg
    close(21)
    deallocate(fa,fb,wg,a,b,shift2d)
    return
end

