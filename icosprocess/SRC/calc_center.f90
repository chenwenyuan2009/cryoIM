...
use common_icosproc
    implicit none
    integer i,j,ix,iy,hx,ky,Nmax(1),ix0,iy0,imgID0
    real cc,z(-1:1,-1:1),dx,dy,centx,centy
    real,allocatable::a1(:),a2(:) 
    real,allocatable::img3d(:),img3di(:)
    complex,allocatable::FFT3d(:)

    allocate(img3d(FFTsize**3))
    allocate(a1(FFTsize**2),a2(FFTsize**2))
...
!    read(21) mrc
    ...
    FFT3d=cmplx(img3d,.0)
    call FFTNEW(4)
    call intialplanCmplx3d(FFT3d,FFT3d,FFTsize,FFTsize,FFTsize)
    call shift3dComplex(FFT3d,FFTsize,FFTsize,FFTsize)
    print *,'Doing 3d FFT!'
    call FFTcmplx3d_f(FFT3d,FFT3d,FFTsize,FFTsize,FFTsize)
    call shift3dComplex(FFT3d,FFTsize,FFTsize,FFTsize)
    ...
    do i=first,last
        ...
        if(abs(centx).ge.float(FFTsize)/16.0) centx=.0
        if(abs(centy).ge.float(FFTsize)/16.0) centy=.0
