...
subroutine cal_crosslin_PR_omega(Nomega,PR1_omega)
    ...
    real,allocatable::alpha1(:),alpha2(:),Fcut1(:),a1(:),b1(:),a2(:),b2(:)
    real,allocatable::FT_commlin1_omega(:,:,:,:),FT_commlin2(:,:,:)
    allocate(alpha1(Nlin),alpha2(Nlin),Fcut1(minR:maxR))
    allocate(a1(FFTsize**2),b1(FFTsize**2),a2(FFTsize**2),b2(FFTsize**2))
    allocate(FT_commlin1_omega(2,minR:maxR,nlin,0:Nomega),FT_commlin2(2,minR:maxR,Nlin))
...
!            call img2d_fill_lin(Nlin,a1,b1,FFTsize,minR,maxR,alpha1-step_angle*iomega,FT_commlin1)
    ...
    deallocate(FT_commlin1_omega,alpha1,alpha2,Fcut1,a1,b1,a2,b2)
